...
class SyntheticTableEvaluator:
    ...
    def __init__(self, real: Table, res_dir: str = 'eval_res',
        ...
        **Args**:
        ...
        if eval_stats:
            self._metrics['stats'] = StatsMetric(real, statistical_metrics)
        if eval_corr:
            self._metrics['corr'] = CorrMatMetric(real, corr_mean, corr_smooth)
        if eval_invalid_comb:
            self._metrics['comb'] = InvalidCombMetric(real, invalid_comb)
        if eval_detect:
        ...
        if eval_clf:
            self._metrics['clf'] = MLClfMetric(
                real, models=clf_models, tasks=clf_tasks, run_default=clf_run_default,
                mean=clf_mean, smooth=clf_smooth
            )
        if eval_reg:
            self._metrics['reg'] = MLRegMetric(
                real, models=reg_models, tasks=reg_tasks, run_default=reg_run_default,
                mean=reg_mean, smooth=reg_smooth
            )
        if eval_card:
            self._metrics['card'] = CardMetric(real, scaling)
        if eval_degree:
            self._metrics['degree'] = DegreeMetric(
                real, count_on=count_on,
    ...
    def evaluate(self, synthetic: SyntheticTable, save_to: Optional[str] = None):
        """
        ...
        **Args**:
        ...
        self._result = {
            n: v.evaluate(synthetic, os.path.join(save_to, n) if save_to is not None else None)
